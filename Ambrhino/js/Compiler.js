...
fn: function () {
    ...
    try {
        ...
        if (e.name === "stReturn" && e.selector === "_nextChunk") {
            return e.fn();
        }
        throw e;
    }
...
smalltalk.addMethod(
unescape('_importString_'),
smalltalk.method({
selector: unescape('importString%3A'),
category: 'fileIn',
fn: function (aString) {
    var self = this;
    var aStream = nil;
    aStream = smalltalk.send(smalltalk.StringStream || StringStream, "_on_", [aString]);
    smalltalk.send(self, "_import_", [aStream]);
    return self;
},
args: ["aString"],
source: unescape('importString%3A%20aString%0A%7C%20aStream%20%7C%0A%20%20%20%20aStream%20%3A%3D%20StringStream%20on%3A%20aString.%0Aself%20import%3A%20aStream.'),
messageSends: ["on:", "import:"],
...
fn: function (aNode) {
    var self = this;
    smalltalk.send(smalltalk.send(aNode, "_temps", []), "_do_", [function (each) {var temp = nil;temp = smalltalk.send(self, "_safeVariableNameFor_", [each]);smalltalk.send(self['@tempVariables'], "_add_", [temp]);return function ($rec) {smalltalk.send($rec, "_nextPutAll_", [smalltalk.send(smalltalk.send("var ", "__comma", [temp]), "__comma", [unescape("%3Dnil%3B")])]);return smalltalk.send($rec, "_lf", []);}(self['@stream']);}]);
